  ...
  fetch("https://ddragon.leagueoflegends.com/cdn/16.5.1/data/ja_JP/item.json")
    ...
    .then((data) => {
      ...
      const blacklist = [
        ...
        "星空のマント", "天帝の剣", "ベイガーの超越タリスマン", "ゼファー",
        "ガーゴイルストーンプレート", "花咲く夜明けの剣", "シャッタークイーンクラウン",
        "ギャンブラーの剣", "冷酷な一撃", "肉食らう者","アナセマ チェイン","オブシディアン エッジ","キルヒアイス シャード","コラプト ポーション",
        ...
        "ガーディアン ブレード","ガーディアン ハンマー","ガーディアン オーブ","シャッタードクイーン クラウン"
            ];
    ...
    return statValue > 0;
          ...
          .map((item) => (
            ...
            >
              ...
              <p className="text-[10px] font-bold text-yellow-500 text-center line-clamp-2 h-6 leading-tight">{item.name}</p>
              <p className="text-[10px] text-gray-400 font-mono">{item.price} G</p>
            </button>
          ))}
      </div>
    </div>
